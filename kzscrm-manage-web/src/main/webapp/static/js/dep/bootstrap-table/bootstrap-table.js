...
!function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        //xyx add 
        onQueryOk: function (data) {return false;}
    ...
    //xyx add
    BootstrapTable.ROW_DEFAULTS = {
    	formattersectr : undefined,
    	formatterfirtr : undefined
    };
    ...
    BootstrapTable.prototype.initTable = function () {
        ...
        this.options.rows = $.extend([], rows, this.options.rows);
        $.each(this.options.rows,function(i,row){
        	that.options.rows[i] = $.extend({}, BootstrapTable.ROW_DEFAULTS,
                    {field: i}, row);
        });
        //xyx add end
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        html.push('</div>',
        	'<div class="pull-right pagination">',//xyx add search-page
        	'<ul class="pagination">',//xyx add search-page
        	'<li class="page-search-num"><input type="text" name="pagenum" style="width:30px;margin-bottom:0px;" class="pull-left"></li>',//xyx add search-page
        	'<li class="page-search"><a href="javascript:void(0)">Go</a></li>',//xyx add search-page
        	'</ul></div>',//xyx add search-page
        ...
        var $search = this.$pagination.find('.page-search a');//xyx add search-page
        var $searchPageNum = this.$pagination.find('input[name=pagenum]');//xyx add search-page
        ...
        $search.off('click').on('click',function(){
        	$this.onSearchPageNumber($searchPageNum);
        });
    };
    ...
    //xyx add search page num
    BootstrapTable.prototype.onSearchPageNumber = function (event) {
    	var num = 0;
    	if(event)
    		num = event.val();
        if (num && Number(num) > 0) {
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            //xyx扩展代码 多行tr 在单行tr之前
            var f = that.options.rows[0].formatterfirtr;
            if(typeof f === 'string'){
            	
            	f = window[f];
                if(typeof f === 'function'){
                	 var rowsFun = f.apply([],[item, i]);
                     if(rowsFun && rowsFun!= null && rowsFun != undefined && rowsFun != ''){
                    	 html.push(rowsFun);
                     }
                }
            }
                ...
                if (that.options.columns[j].checkbox || that.options.columns[j].radio) {
                    ...
                    text = that.options.cardView ?
                        ['<div class="card-view">',
                            ...
                            '</div>'].join('') :
                        [sprintf('<td%s %s %s %s>', id_, class_, style,(that.options.rows[0].formattersectr && j==that.header.fields.length-1 ? 'rowspan="2"' : '')),//xyx add the four args
                            value,
                            '</td>'].join('');
            ...
            html.push('</tr>');
            //xyx扩展代码,如多行 在单行tr之后
//             var rowsFun = calculateObjectValue(that.options, that.options.rows[0].formattersectr, [item, i], "");
            var f = that.options.rows[0].formattersectr;
            if(typeof f === 'string'){
            	f = window[f];
                if(typeof f === 'function'){
                	 var rowsFun = f.apply([],[item, i]);
                     if(rowsFun && rowsFun!= null && rowsFun != undefined && rowsFun != ''){
                    	 html.push('<tr><td style="text-align:center;" colspan="'+(this.header.fields.length-1)+'">'+rowsFun+'</td></tr>'  );
                     }
                }
            }
        }
    ...
    BootstrapTable.prototype.initServer = function (silent) {
        ...
        if (this.options.queryParamsType === 'limit') {
            params = {
                ...
                start: params.pageSize * (params.pageNumber - 1) + 1,//修改分页起始参数 for gzl
        ...
        var timestampParam = (new Date()).valueOf();  
        
        $.ajax({
            type: this.options.method,
            url: this.options.url+"&t="+timestampParam,
            data: data,
            cache: false,
            ...
            success: function (res) {
                ...
                //xyx add onloadsuccess
                var f = that.options['onloadsuccess'];
                if(typeof f === 'string'){
                	f = window[f];
                    if(typeof f === 'function'){
                    	 f.apply([],[data]);
