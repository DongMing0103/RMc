...
function PageInit() {
    ...
    this.readURL = function () {
    	var _imghtml='<img src="" id="cut-img" class="img-responsive img-thumbnail cut-imgClass" width="558" height="309" style="padding:0;height: 0!important;border:none">';
        ...
        /*if (input[0].files && input[0].files[0]) {*/
            ...
            reader.onload = function (e) {
            	//$(".selectimg").remove();
            	//$(".selectimg").append(' <img alt="" src="/static/images/showimg.jpg" id="cut-img" class="img-responsive img-thumbnail"/>');
            	var img = $('#cut-img');
                img.removeAttr('src');
                /*img.attr('src', e.target.result);*/
                
                /*裁剪图片按比例缩放*/
                var width = 558;//img.attr('width');//区域宽度
                var height = 309;//img.attr('height');//区域高度
                ...
                img.attr('src', src);
                imgObj.src = src;
                $(imgObj).hide().load(function() {
                		imgWidth = imgObj.width;
                		imgHeight = imgObj.height;
                		console.log("img========" + imgWidth + "====" + imgHeight);
                		imgRatio = imgWidth / imgHeight;//实际宽高比
                		if(ratio > imgRatio){
                			showWidth = (height * imgWidth) / imgHeight;
                			showHeight = height;
                		}else{
                			showWidth = width;
                			showHeight = (width * imgHeight) / imgWidth;
                		}
                		console.log("showimg========" + showWidth + "====" + showHeight);
                		var marginRight = 0;
                        var marginBottom = 0; 
                        if(width > showWidth){
                        	marginRight = (width - showWidth) / 2;
                        }
                        if(height > showHeight){
                        	marginBottom = (height - showHeight) / 2;
                        }
                        
                		img.css('width', showWidth).css('margin-left', marginRight);
                		img.css('height', showHeight).css('margin-bottom', marginBottom);
                		...
                		/*img.Jcrop({
                            ...
                            api = this;
                            $(".jcrop-holder").css('width', showWidth).css('margin-left', marginRight);
                            $(".jcrop-holder").css('height', showHeight).css('margin-bottom', marginBottom);
                            $(".jcrop-tracker").css('width', showWidth).css('margin-left', marginRight);
                    		$(".jcrop-tracker").css('height', showHeight).css('margin-bottom', marginBottom);
                    		$(".jcrop-holder img").css('width', showWidth);
                    		$(".jcrop-holder img").css('height', showHeight);
                        });*/
