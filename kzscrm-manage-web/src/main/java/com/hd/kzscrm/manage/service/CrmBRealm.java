...
//		accountPO.setMobilephone(username);
		...
		else if(po.getUserType().equals(CrmAccountUserType.AGENT.getCode())){
			CrmAgentPO crmAgentPO = iCrmAgentService.getByUserId(po.getId());
			if(BeanUtils.isEmpty(crmAgentPO)){
				throw new BusinessException("未找到指定的代理商信息");
			}
			SecurityUtils.getSubject().getSession().setAttribute(Consts.SESSION_AGENT_KEY, crmAgentPO);
		}else if(po.getUserType().equals(CrmAccountUserType.BUSINESS.getCode())
				||po.getUserType().equals(CrmAccountUserType.BUSINESS_MANAGER.getCode())){
			CrmBusinessPO crmBusinessPO= iCrmBusinessService.getByUserId(po.getId());
			if(BeanUtils.isEmpty(crmBusinessPO)&&po.getUserType()==CrmAccountUserType.BUSINESS.getCode()){
				throw new BusinessException("未找到指定的业务员信息");
			}else if(BeanUtils.isEmpty(crmBusinessPO)&&po.getUserType()==CrmAccountUserType.BUSINESS_MANAGER.getCode()){
				throw new BusinessException("未找到指定的业务经理信息");
			}
			if(BeanUtils.isNotEmpty(crmBusinessPO) && !CommUtil.parseInteger(crmBusinessPO.getJobStatus()).equals(1)){
				throw new BusinessException("该账号已注销");
			}
			SecurityUtils.getSubject().getSession().setAttribute(Consts.SESSION_BUSINESS_KEY, crmBusinessPO);
		}
