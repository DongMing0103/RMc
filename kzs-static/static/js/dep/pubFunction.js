        ...
        function(){
            ...
            switch(this.type){
                ...
                case 'textarea':
                	...
                	break;
                case 'checkbox':  
                	(data[name] != undefined) && data[name] == 1 && (this.checked = true) && $(this).val(data[name]);
                case 'radio': 
 ...
 * */
function loadCompanyData4UZy(async,selectDom,companyName,companyId, defaultCompanyId){
	if(async == undefined || async == null){
		async = true;
	}
    var timestamp = (new Date()).valueOf();  
	...
	var ajax = $.ajax({
			        cache: true,
			        type: "POST",
			        url:"/LinkCompanyUser.do?action=query4uZy&t="+timestamp,
			        data:{"companyName":companyName||'',"id":companyId||'',pageSize:9999999},
			        async: async,//是否同步,默认异步
			        dataType:"json",
			        error: function(request,info,e) {
			        ...
			        success: function(data) {
			        	if(data  && data.total){
			    			...
			    			layer.alert('<p style="text-align: center; line-height: 30px;">您尚未认证三证<br><a href="/LinkCompanyUser.do?action=init" style="color: #f60;">点击认证</a></p>');
			    			return false;
			    		}
			        }
			    });
...
function loadCompanyData4U(async,selectDom,companyName,companyId, defaultCompanyId){
	...
	var ajax = $.ajax({
			        cache: true,
			        type: "POST",
			        url:"/LinkCompanyUser.do?action=query4u&t="+timestamp,
			        data:{"companyName":companyName||'',"id":companyId||'',pageSize:9999999},
			        async: async,//是否同步,默认异步
			        dataType:"json",
			        error: function(request,info,e) {
		...
		ajax:{
			...
			results: function(data, page, query){
				if(data.rows){
					...
					data.rows.splice(0,0,{id:0, areaName: query.term || '全部', areaCode:''});
					...
					return{
						results: {id:0, areaName: query.term || '全部', areaCode:''}
					};
				}
			}
		}
...
function cateSelect(dom, defaultName, defaultValue, remark){
	$(dom).val('');
	$(dom).select2({
		...
		ajax:{
			...
			results: function(data, page, query){
				if(data.rows){
					if(query){
						if(!query.term){
							data.rows.splice(0,0,{cateName: remark || query.term || '全部', id:''});
						}
					}
					//请求后返回的结果
					return{
					...
					return{
						results: {cateName:remark || query.term || '全部', id:''}
					};
				}
			}
		}
 ...
 * 
 * */	
function loadCateGoodsDataBySelect2(async,cateSelectDom, goodsIdDom, goodsSelectDom,  defaultCateId, defaultGoodsId, defaultGoodsName,defaultCateName,cateNameDom){
	if(async == undefined || async == null){
		async = true;
	}
	...
	if(cateSelectDomList.length > 0){
		$.each(cateSelectDomList,function(index, cateSelect){
			...
			$(goodsIdDomList[index]).select2({
				placeholder: defaultGoodsName || '请选择牌号',  //默认显示的数据
				allowClear: true,
				formatResult: function(result, container, query, escapeMarkup){ //返回结果时做的操作
				...
				formatSelection: function(item){//选择下拉框值时做的操作
					...
					if(goodsSelectDomList.length > 0){
						$(goodsSelectDomList[index]).val(item.goodsName == '全部' ? '' : item.goodsName); 
					}
					$(goodsIdDomList[index]).blur();
			...
			$(cateSelectDomList[index]).select2({
				placeholder: defaultCateName || '请选择品种',  //默认显示的数据
				allowClear: true,
				formatResult: function(result, container, query, escapeMarkup){ //返回结果时做的操作
					$(cateSelect).blur();
					return result.cateName;
				...
				formatSelection: function(item){//选择下拉框值时做的操作
					...
					if(cateNameDomList.length > 0){
						$(cateNameDomList[index]).val(item.cateName == '全部' ? '' : item.cateName); 
					}
					//品种改变，牌号跟着改变
 ...
 */
function loadCompanyDataBySelect2(selectDom,isLink,defaultCompanyName,defaultCompanyId, companyName,isNeedAll,unNeedIndustryId){
	if(selectDom && selectDom != undefined){
		var url = "/Company.do?action=query";
		if(isLink){
			url = "/LinkCompanyUser.do?action=query4u";
		}
		selectDom.val('');
		selectDom.select2({
			placeholder: defaultCompanyName || '请选择公司',
			allowClear: false,
			...
			ajax:{
				...
				results: function(data, page, query){
					var list = [];
					if(isNeedAll){
						list.push({id : '',companyName:'',placeholder:'全部公司'});
					}
					//请求后返回的结果
					if(data&&data.rows&&data.rows.length > 0){
						...
						return {
							results: [{id : '',companyName: query.term}]
						};
					}
				}
			}
...
function loadCustomerManager(async, selectDom, selectNameDom, defaultValue, defaultName){
	...
	$(selectDom).select2({
		...
		ajax:{
			...
			results: function(data, page, query){
				var list = [];
				list.push({id : '',name: query.term || '全部',placeholder:'全部客户经理'});
				//请求后返回的结果
				if(data&&data.rows&&data.rows.length > 0){
					$.each(data.rows,function(i,o){
						list.push(o);
...
function formatDate5(d) {
	var hours = null;//(d.getHours()<=9?'0'+d.getHours() : d.getHours());
	var minutes = null;
	if(d.getHours()<=9){
		if(d.getHours()<8){
			hours = random(8,18);
...
function newToDate(value){
	...
	if(difference < 0){ //是未来时间，显示前一天的月-天
		...
		return (yesterday.getMonth() + 1) + "-" + (yesterday.getDate()<=9?'0'+yesterday.getDate() : yesterday.getDate());
	}
 ...
 * @returns {String}
 */
function newToDate2(value,date){
	if(value == "" || value == undefined){
		return "";
	}
	var time = new Date(value);
...
/**
 * 将数据显示两位小数。如果原始数据是整数的，会加入小数点.如果原始数据带小数的，会将其转换成只显示两位小数点.<br/>
 * 小数点不足两位的会补齐，超过两位的会四舍五入
 * @param str
 */
function showTwoDecimal3(str){
	var baseNum = str + '';
	try {
		var strArr = baseNum.split(".");
		if(strArr.length > 1){
...
var mydatatype = {
	...
	"str":/^[\w\.]+$/,//只包含字符,数字的字符串
	"str6-20":/^[\w\.]{6,20}$/, ////只包含字符,数字6-20字符串
	...
	"integer1-3": /^([0-9]\d{1,3}|\d)?$/, //匹配金额类型
	"gt0":/^[1-9]\d*$/ ,// 大于0
	"price3-3": /^([0-9]\d{1,3}|\d)(\.\d{1,3})$/, //匹配金额类型
	"price0-3": /^\d+(\.\d{1,3})?$/   //匹配金额类型 3位小数
}
$.Tipmsg.w = {
	...
	LoadCateGoodsFactory.prototype.cateSelect2 = function(){
		...
		$.each(args.cateSelectDomList, function(index, cateSelect){
			...
			$(args.cateSelectDomList[index]).select2({
				placeholder: args.defaultCateName || '请选择品种',  //默认显示的数据
				allowClear: true,
...
// 根据行业ID 获取行业名称
function getNameForIndustryId(industryId,industryList) {
	if (industryId && industryList) {
		for(var i = 0;i<industryList.length;i++){
...
//自动登录
function iframeToLogin(username,password,webList){
	var host = window.location.host;
	if(webList != null && webList != "" && webList != "undefined"){
		//遍历集合
		for(var i=0;i<webList.length;i++){
			if("http://"+host != webList[i]){
				if("http://g.jm.com" == webList[i] || "http://eng.jm.com" == webList[i] || "http://s.jm.com" == webList[i] || "http://b.jm.com" == webList[i]){ //三馆项目:国家馆、国家馆英文版、省馆、品牌馆
...
//定时刷新
/*function timeRefreshHttp(webList){
 ...
 *交货时间 交易员中心
 */
function deliveryDaysToTime(deliveryDays){
	var d = new Date(deliveryDays);
	var month = ((d.getMonth()+1)<=9?'0'+(d.getMonth()+1) : (d.getMonth()+1));
	var date = (d.getDate()<=9?'0'+d.getDate() : d.getDate());
	return d.getFullYear() + "." + month + "." + date;
}

//设置cookie 加载类型 解决刷新页面无法保持在原先页面问题 type为页面类型
function setTraderCookie(type){
	var sysDate = new Date();
	//设置有效时间 30分钟
	sysDate.setTime(sysDate.getTime() + (30 * 60 * 1000));
	$.cookie("loadType", type, {expires: sysDate});
}
