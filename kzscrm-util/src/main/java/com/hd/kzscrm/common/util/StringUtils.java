...
public class StringUtils {
    ...
    /**
     * 判断指定字符串的字符个数,其中中文算两个字符
     * @param str
     * @return
     * @author zhangyz created on 2015-4-22
     */
    public static int calcFactCharNum(String str) {
        int total = 0;
    ...
    /**
     * 返回前面指定字符个数的字串，如果是中文算两个字符。如果最后一个刚好是汉字超过指定个数，则达不到最终长度
     * @param str
     * @param charNum
     * @return
     * @author zhangyz created on 2015-4-22
     */
    public static String subFactString(String str, int charNum) {
        int total = 0;
     ...
     * @param maxbit
     * @return
     * @author zhangyz created on 2014-12-4
     */
    public static final String genRadmonString(int maxbit) {
        Random secretRandom1 = new Random();
        Long secret1 = secretRandom1.nextLong();
        return formatToZeroBefore(secret1, maxbit);
    }
    ...
    /**
     * 将数字格式化为后空位补0的字符串，如果数字位数大于允许最大位数，后面数字将被截去
     * @param num
     * @param maxbit
     * @return
     * @author zhangyz created on 2015-10-10
     */
    public static final String formatToZeroEnd(String strNum, int maxbit) {
        StringBuffer buf = new StringBuffer(maxbit);
        if (strNum.length() < maxbit) {
            buf.append(strNum);
            for (int i = 0; i < (maxbit - strNum.length()); i++)
            ...
            buf.append(strNum.substring(0, maxbit));
        return buf.toString();
    }

    public static final String formatToZeroBefore(Integer num, int maxbit) {
        StringBuffer buf = new StringBuffer(maxbit);
   ...
   /**
    ...
    /**
     * 获取以“.”分割的字符串的最后一个子串
     * @param fullString 要分割的完整字符串
     * @return
     */
    public static String getLastSegment(String fullString){
        if ((fullString == null) || (fullString.length() == 0))
            return null;
        else{
            String [] xxs = org.apache.commons.lang.StringUtils.split(fullString,".");
    ...
    public static String getShortClassName(String className){
        ...
        if(className != null && className.length()!=0){
            splits = org.apache.commons.lang.StringUtils.split(className,".");
    ...
    public static String getPackageName(String className){
        ...
        if(iLast <= 0)
            ...
            return className.substring(0,iLast);
        }
    }
    ...
    public static void main(String args[]) {
        System.out.println(subFactString("aa3aaa",5));
    ...
    //将以","隔开的id集合的String转为list集合
    public static List<Long> idsStrToList(String idsStr){
    	if(null == idsStr || "".equals(idsStr)){
    		return null;
    	}
    	idsStr = idsStr.replace(" ", "");//去除空格
    	if("".equals(idsStr)){
    		return null;
    	}
    	...
    	for (String string : strs) {
			Long id = StringUtil.parseLong(string);
			ids.add(id);
		}
